...
```sql
MariaDB [dhairya]> select ename,(sal*12) as annual_salary from employee
    -> order by sal desc;
+--------+---------------+
| ename  | annual_salary |
+--------+---------------+
...
```sql
MariaDB [dhairya]> select sal,(sal*0.15) as hra,
    -> (sal*0.10) as da,
    -> (sal*0.05) as pf
    -> from employee
...
```sql
MariaDB [dhairya]> select ename,job,sal,(0.10*sal+sal) as updated_salary
    -> from employee
    -> where comm is null ;
+--------+-----------+------+----------------+
...
MariaDB [dhairya]> select ename,(0.20*sal+sal) as updated_salary
    -> from employee
    -> where sal>3000;
+-------+----------------+
...
```sql
MariaDB [dhairya]> select ename,sal
    -> from employee
    -> where sal like '___';
+-------+------+
| ename | sal  |
